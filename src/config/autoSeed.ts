      ...
      await Treatment.create([
        {
          ...
          procedureSteps: [
            JSON.stringify({ step: 'Oil Selection', detail: 'Physician-directed medicated oil choice based on your dosha and condition.' }),
            JSON.stringify({ step: 'Application', detail: 'Warm medicated oil massage by two trained therapists in dedicated rooms using rhythmic synchronized strokes.' }),
            JSON.stringify({ step: 'Swedana (Optional)', detail: 'Herbal steam bath may follow to enhance oil absorption and open channels.' }),
            JSON.stringify({ step: 'Rest & Recovery', detail: 'Post-therapy rest period; may be followed by a warm bath as advised by physician.' }),
          ],
          assignedBranchIds: [kattakadaBranch._id, kowdiarBranch._id],
          isFeatured: true,
          status: 'published',
        },
        {
          ...
          procedureSteps: [
            JSON.stringify({ step: 'Preparation & Positioning', detail: 'Comfortable supine positioning on the Droni (therapy table) with forehead exposed and eyes covered.' }),
            JSON.stringify({ step: 'Dhara (Oil Stream)', detail: 'Steady, continuous stream of warm prescribed medicated oil or liquid poured from a specific height across the forehead for a set duration.' }),
            JSON.stringify({ step: 'Scalp & Hair Care', detail: 'Gentle scalp and hair treatment to remove residual oil and promote further absorption.' }),
            JSON.stringify({ step: 'Rest & Observation', detail: 'Quiet rest in a low-stimulus environment afterwards; avoid sudden stimulation or cold exposure.' }),
          ],
          assignedBranchIds: [kattakadaBranch._id, kowdiarBranch._id],
          isFeatured: true,
          status: 'published',
